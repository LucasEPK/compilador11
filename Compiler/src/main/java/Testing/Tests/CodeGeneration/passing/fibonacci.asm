...
Fibonacci_sucesion_fib:
	...
	# Fin condición while
	bne $v0, 1, default_while_fin_l11c11	#si la condición del while no es verdadera salta todo el while
	# Cuerpo while
	# Condicional if
	...
	jal default_equal
	# If con un else
	bne $v0, 1, default_if_else_skip_l12c9	# Si la condición es falsa (acumulador!=1) se skipea el then y se va al else
	...
	pop	# Pop de parametro 0
	# FIN desapilado del RA completo de la función llamada
	j default_if_else_fin_l12c9		# Acá se salta al fin del ifelse porque sino ejecutariamos codigo del else
	default_if_else_skip_l12c9:
	...
	jal default_equal
	# If con un else
	bne $v0, 1, default_if_else_skip_l17c9	# Si la condición es falsa (acumulador!=1) se skipea el then y se va al else
	# Acá va el cuerpo (then)
	#Guardamos el CIR en $s4
	la $s4, ($v0)	#Guardamos el CIR en $s4
	lw $t0, 8($fp)		# Cargamos el CIR en $t0
	...
	pop	# Pop de parametro 0
	# FIN desapilado del RA completo de la función llamada
	j default_if_else_fin_l17c9		# Acá se salta al fin del ifelse porque sino ejecutariamos codigo del else
	default_if_else_skip_l17c9:
	# Acá va el cuerpo (else)
	#Guardamos el CIR en $s4
	la $s4, ($v0)	#Guardamos el CIR en $s4
	lw $t0, 8($fp)		# Cargamos el CIR en $t0
	...
	pop	# Pop de parametro 0
	# FIN desapilado del RA completo de la función llamada
	default_if_else_fin_l17c9:
	default_if_else_fin_l12c9:
	lw $t0, 8($fp)		# Cargamos el CIR en $t0
	lw $v0, 8($t0)
	addiu $v0,$v0, 1 #++
	lw $t0, 8($fp)	# Cargamos el CIR en $t0
	...
	la $v0, ($t0)	# Cargamos el CIR en $v0 para ser guardado por funciones
	# Fin cuerpo while
	j default_while_l11c11	# vuelve al pricipio del while a chequear la condición
	default_while_fin_l11c11:
	# Return
	lw $t0, 8($fp)		# Cargamos el CIR en $t0
	lw $v0, 4($t0)
	la $t9,($v0) #cargo en $t9 el valor de retorno
